...
public class GraphAlgorithms {
    ...
    public static void dfsWeighted(WeightedGraph graph, int from, Consumer<Integer> visitor) {
        boolean[] visited = new boolean[graph.vertexCount()];
        Stack<Integer> stack = new Stack<Integer>();
        stack.push(from);
        visited[from] = true;
    ...
    @FunctionalInterface
    interface Consumer1{
        void accept(WeightedGraph.WeightedEdgeTo edge, int from);
    }

    private static class Edge{
        WeightedGraph.WeightedEdgeTo edge;
        int from;
        boolean visited = false;
        Edge(WeightedGraph.WeightedEdgeTo edge, int from) {
            this.edge = edge;
            this.from = from;
        }
    }

    public static void dfs(WeightedGraph graph, int from, Consumer1 visitor) {
        //boolean[] visited = new boolean[graph.edgeCount()];
        ...
        while (!stack.empty()) {
            ...
            if (curr != null) {
                from = curr.edge.to();
                visitor.accept(curr.edge, curr.from);
            }
            for (WeightedGraph.WeightedEdgeTo v : graph.adjacenciesWithWeights(from)) {
                if (!v.getVisited()) {
                    stack.push(new Edge(v, from));
                    v.setVisited(true);
                    for (WeightedGraph.WeightedEdgeTo from1 : graph.adjacenciesWithWeights(v.to())){
                        if (from1.to()==from && !from1.getVisited()){
                            from1.setVisited(true);
                        }
                    }
                }
            }
    ...
    public static MinDistanceSearchResult dijkstra(WeightedGraph graph, int source) {
        ...
        d[source] = graph.getAdjCount(source);
        from[source] = -1;
        //int prev = -1;
        for (int i = 0; i < n; i++) {
            ...
            for (WeightedGraph.WeightedEdgeTo v : graph.adjacenciesWithWeights(curr)) {
                if (d[curr] + v.weight() + graph.getAdjCount(v.to()) < d[v.to()]) {
                    d[v.to()] = d[curr] + v.weight() + graph.getAdjCount(v.to());
                    from[v.to()] = curr;
